...
namespace Metatron.Core
{
    ...
    internal class MissionCache : ModuleBase, IMissionCache
    {
        ...
        private readonly string WindowName_Journal = "journal",
            Button_RequestMission = "Request Mission",
            Button_ViewMission = "View Mission",
            //BUTTON_BUY_DATACORES = "UI/Agents/Dialogue/Buttons/BuyDatacores",
            Button_CompleteMission = "Complete Mission",
            ...
            Tag_NoDeclinePenalty = "we won’t penalize you if you decline it.";
        ...
        internal MissionCache(IIsxeveProvider isxeveProvider, IEveWindowProvider eveWindowProvider)
        {
            ...
            _configDirectory = Metatron.ConfigDirectory;
        }
        ...
        private void SaveMissionCache(bool sendCallback)
        {
            ...
            _missionDbFileName = string.Format("{0}{1}", Metatron.MeCache.Name, _missionDbFileSuffix);
            _missionDbFilePath = string.Format("{0}\\{1}", _configDirectory, _missionDbFileName);

            Metatron.FileManager.QueueOverwriteSerialize(_missionDbFilePath, new List<CachedMission>(_cachedMissions), sendCallback ? _writeCallback : null);
        }
        ...
        private void LoadMissionCache()
        {
            ...
            Metatron.FileManager.QueueDeserialize(_missionDbFilePath, _loadCallback);
        }
                ...
                case UpdateCacheStates.OpenJournal:
                    ...
                    using (var journalWindow = _eveWindowProvider.GetWindowByName(WindowName_Journal))
                    {
                        ...
                        else
                        {
                            ...
                            if (Metatron.MeCache.AgentMissions.Count == 0)
                            {
        ...
        private bool CheckMissions(int agentId, bool fastUpdate)
        {
            ...
            if (agentId > 0)
            {
                currentAgentMission = Metatron.MeCache.AgentMissions.FirstOrDefault(agentMission => agentMission.AgentID == agentId);
            ...
            else
            {
                ...
                if (fastUpdate)
                {
                    ...
                    for (var cmIdx = 0; cmIdx < _cachedMissions.Count; cmIdx++)
                    {
                        ...
                        var matched = false;
                        foreach (var agentMission in Metatron.MeCache.AgentMissions.Where(agentMission => agentMission.AgentID == cachedMission.AgentId))
                        {
                            //set the flag
                            matched = true;
                    ...
                    for (var amIdx = 0; amIdx < Metatron.MeCache.AgentMissions.Count; amIdx++)
                    {
                        var agentMission = Metatron.MeCache.AgentMissions[amIdx];
                ...
                if (Metatron.MeCache.AgentMissions.Count > _lastMissionIndex)
                {
                    currentAgentMission = Metatron.MeCache.AgentMissions[_lastMissionIndex];
                }
                else
                    return true;
            }
            ...
            var cachedAgent = Metatron.AgentCache.GetCachedAgent(currentAgentMission.AgentID);
            ...
            switch (_checkMissionState)
            {
                ...
                case CheckMissionStates.OpenAgentConversation:
                    ...
                    using (var agentConversationWindow = GetAgentConversationWindow(cachedAgent))
                    {
                        var agent = Metatron.AgentCache.GetAgent(cachedAgent);
                        ...
                        if (LavishScriptObject.IsNullOrInvalid(agentConversationWindow))
                        {
                            ...
                            Metatron.ModuleManager.DelayPulseByHighestTime(Delay_LongLoad);
                        }
                        else
                        {
                            //make sure we don't need to ViewMission
                            var dialogStrings = GetDialogResponses(cachedAgent);
                            foreach (var dialogString in dialogStrings.Where(dialogString => dialogString.Contains(Button_ViewMission)))
                            {
                                //Damn, we do. Say it.
                ...
                case CheckMissionStates.OpenMission:
                    using (var missionWindow = GetAgentConversationWindow(cachedAgent))
                    {
                ...
                case CheckMissionStates.Cleanup:
                    ...
                    if (Metatron.MeCache.AgentMissions.Count > _lastMissionIndex + 1)
                    {
                        if (fastUpdate)
                            return false;
        ...
        public ConversationStateResults GetMissionFromAgent(int agentId)
        {
            ...
            var cachedAgent = Metatron.AgentCache.GetCachedAgent(agentId);
            var agent = Metatron.AgentCache.GetAgent(cachedAgent);
            var dialogStrings = GetDialogResponses(cachedAgent);
            ...
            switch (_getMissionState)
            {
                ...
                case GetMissionStates.OpenAgentConversation:
                    ...
                    using (var agentConversationWindow = GetAgentConversationWindow(cachedAgent))
                    {
                        ...
                        else
                        {
                            ...
                            dialogStrings = GetDialogResponses(cachedAgent);
                            foreach (var dialogString in
                                dialogStrings.Where(
                                    dialogString =>
                                    dialogString.Contains(Button_ViewMission) || dialogString.Contains(Button_RequestMission)))
                            {
                                SayDialogStringToAgent(dialogString, cachedAgent);
                                return ConversationStateResults.Incomplete;
                            }
                    ...
                    if (UpdateCache(cachedAgent.Id))
                    {
                        ...
                        if (cachedMission == null)
                        {
                            LogMessage(methodName, LogSeverityTypes.Standard,
                                "Unable to find a cached mission from this agent. This usually means accepting the mission failed due to a dialog box. Manually accept the mission, click the \"do not show this again\" check box on the dialog, and resume Metatron.");
                        ...
                        {
                            //Check for unsupported missions
                        ...
                        if (Metatron.Config.MissionConfig.IgnoreMissionDeclineTimer || DateTime.Now.CompareTo(cachedAgent.NextMissionDeclinable) >= 0)
                        {
                ...
                case GetMissionStates.AcceptMission:
                    var acceptStringFound = dialogStrings.Contains(Button_Accept);
                    foreach (var dialogString in
                        dialogStrings.Where(dialogString => dialogString.Contains(Button_Accept)))
                    {
                        SayDialogStringToAgent(dialogString, cachedAgent);
                    }
                    ...
                    {
                        LogMessage(methodName, LogSeverityTypes.Debug, "Accept Mission dialog string not found, assuming mission already accepted.");
                        var cachedMission = GetCachedMissionForAgentId(cachedAgent.Id);
                ...
                case GetMissionStates.DeclineMission:
                    foreach (var dialogString in
                        dialogStrings.Where(dialogString => dialogString.Contains(Button_Decline)))
                    {
                        var cachedMission = GetCachedMissionForAgentId(cachedAgent.Id);
                        if (!cachedMission.DeclineTimerExempt) cachedAgent.NextMissionDeclinable = DateTime.Now.AddHours(4);
                        ...
                        Metatron.AgentCache.SaveAgentCache();
                        SayDialogStringToAgent(dialogString, cachedAgent);
        ...
        private bool IsOnDefaultBlacklist(CachedMission cachedMission)
        {
            var methodName = "IsOnDefaultBlacklist";
            LogTrace(methodName, "Mission: {0}", cachedMission.Name);

            var defaultMissionBlacklist = new List<string>()
                    {
                        "Anomic Base",
                        "Anomic Team",
                        "Anomic Agent",
            ...
            foreach (string mission in defaultMissionBlacklist)
            {
                if (mission == cachedMission.Name)
                {
                    LogTrace(methodName, "true");
                    return true;
                }
            }
            LogTrace(methodName, "false");
            return false;
        }
        ...
        public bool TurnInMission(CachedAgent cachedAgent)
        {
            ...
            using (var agentConversationWindow = GetAgentConversationWindow(cachedAgent))
            {
                ...
                else
                {
                    ...
                    foreach (var dialogString in
                        dialogStrings.Where(dialogString => dialogString.Contains(Button_ViewMission)))
                    {
                        SayDialogStringToAgent(dialogString, cachedAgent);
                        return false;
                    }
                    ...
                    foreach (var dialogString in
                        dialogStrings.Where(dialogString => dialogString.Contains(Button_CompleteMission)))
                    {
                        SayDialogStringToAgent(dialogString, cachedAgent);
                        return false;
                    }

                    var newDialogStrings = GetDialogResponses(cachedAgent);
                    //Invalidate the currentMission if we actually dropped it
                    if (newDialogStrings.Contains(Button_CompleteMission))
                    {
                        LogTrace(methodName, "Mission still accepted");
                        return false;
                    }
                    else
                    {
                        LogTrace(methodName, "Mission Dropped");
                        ClearMissionFromAgent(cachedAgent.Id);
        ...
        private void SayDialogStringToAgent(string buttonText, CachedAgent cachedAgent)
        {
            var methodName = "SayDialogStringToAgent";
            LogTrace(methodName, "DialogString: {0}, CachedAgent: {1}", buttonText, cachedAgent.Name);

            LogMessage(methodName, LogSeverityTypes.Debug, "Saying \"{0}\" to agent \"{1}\".", buttonText, cachedAgent.Name);

            GetAgentConversationWindow(cachedAgent).Button(buttonText).Press();
            //var buttonCommand = "EVEWindow[byCaption, Agent Conversation - " + cachedAgent.Name + "].Button[" + buttonText + "]:Press";
            //LogTrace(methodName, buttonCommand);
            //var returnInt = LavishScript.ExecuteCommand(buttonCommand);
            //LogTrace(methodName, "Execute command: " + returnInt);
        ...
        private EVEWindow GetAgentMissionWindow(CachedAgent agent)
        {
            ...
            var missionWindowCaptionString = string.Format("Agent Conversation - {0}", agent.Name);
        ...
        private string GetMissionDetails(AgentMission agentMission, CachedAgent agent)
        {
            ...
            using (var eveWindow = GetAgentConversationWindow(agent))
            {
                //Get the detail string. This will be the window's HTML value.
                var detailsString = eveWindow.GetString("ObjectivesHTML");
                LogMessage(methodName, LogSeverityTypes.Debug, "Window HTML Length: {0}, Mission Details length: {1}",
                           eveWindow.GetString("ObjectivesHTML"), detailsString.Length);
                ...
                Metatron.Logging.LogMission(detailsString, expiration, agentMission.Name);
        ...
        private CachedMission ParseMissionDetails(AgentMission agentMission, CachedAgent agent)
        {
            ...
            if (IsMissionDeclineTimerExempt(detailsString))
            {
                cachedMission.DeclineTimerExempt = true;
            }
        ...
        private bool IsMissionDeclineTimerExempt(string details)
        {
            var methodName = "IsMissionDeclineTimerExempt";
            LogTrace(methodName);

            return details.Contains(Tag_NoDeclinePenalty);
        }
        #endregion
        ...
        public AgentMission GetAgentMissionFromAgent(int agentId)
        {
            ...
            foreach (var agentMission in Metatron.MeCache.AgentMissions.Where(agentMission => agentMission.AgentID == agentId))
            {
                return agentMission;
            }
        ...
        private List<string> GetDialogResponses(CachedAgent agent)
        {
            var missionWindow = GetAgentConversationWindow(agent);
            var validButtons = new List<string>();

            // Define the button names.
            var buttonNames = new List<string> { Button_ViewMission, Button_RequestMission, Button_Accept, Button_Decline, Button_CompleteMission };

            foreach (var buttonName in buttonNames)
            {
                var button = missionWindow.Button(buttonName);

                // Check for validity.
                if (button != null && button.IsValid)
                {
                    validButtons.Add(buttonName);
                }
            }
        ...
        private EVEWindow GetAgentConversationWindow(CachedAgent agent)
        {
            var methodName = "GetAgentConversationWindow";
            LogTrace(methodName, "AgentConversation: {0}", agent.Name);

            var missionWindowCaptionString = string.Format("Agent Conversation - {0}", agent.Name);

            var agentMissionWindow = _eveWindowProvider.GetWindowByCaption(missionWindowCaptionString);
            LogTrace(methodName, "IsValid: " + agentMissionWindow.IsValid);
        ...
        public AgentMission GetAgentMission(CachedMission cachedMission)
        {
            ...
            Metatron.Logging.LogTrace(ModuleName, methodName);

            //Iterate a list of AgentMissions and look for the matching one.

            if (Metatron.MeCache.AgentMissions.Count == 0)
            {
                Metatron.Logging.LogMessage(ModuleName, methodName, LogSeverityTypes.Standard, "Error; GetAgentMissions returned no AgentMissions!");
                return null;
            }

            foreach (var agentMission in Metatron.MeCache.AgentMissions)
            {
                if (LavishScriptObject.IsNullOrInvalid(agentMission))
                {
                    Metatron.Logging.LogMessage(ModuleName, methodName, LogSeverityTypes.Standard, "Error: GetAgentMissions returned an invalid AgentMission!");
                    continue;
                }
        ...
        public List<BookMark> GetMissionBookmarks(CachedMission cachedMission)
        {
            ...
            {
                Metatron.Logging.LogMessage(ModuleName, methodName, LogSeverityTypes.Debug, "Error: Could not find AgentMission for CachedMission \"{0}\" for agent with ID {1}.",
                    cachedMission.Name, cachedMission.AgentId);
            ...
            {
                Metatron.Logging.LogMessage(ModuleName, methodName, LogSeverityTypes.Debug, "Error: AgentMission.GetBookmarks returned null.");
        ...
        public bool IsMissionAcceptible(CachedMission cachedMission)
        {
            ...
            var blacklistMatch = Metatron.Config.MissionConfig.MissionBlacklist.FirstOrDefault(missionName => cachedMission.Name.Contains(missionName, StringComparison.InvariantCultureIgnoreCase));
            if (blacklistMatch != null)
            {
                Metatron.Logging.LogMessage(ModuleName, methodName, LogSeverityTypes.Debug, "Mission \"{0}\" matches blacklisted name \"{1}\".",
                    cachedMission.Name, blacklistMatch);
                return false;
            }

            //// Check if mission is on default blacklist
            //if (IsOnDefaultBlacklist(cachedMission))
            //{
            //    Metatron.Logging.LogMessage(ModuleName, methodName, LogSeverityTypes.Debug, "Mission \"{0}\" is on the default blacklist.", cachedMission.Name);
            //    return false;
            //}

            var avoidLowSec = Metatron.Config.MissionConfig.AvoidLowSec;
            if ((!avoidLowSec || !cachedMission.IsLowSecurity) && cachedMission.HaveRoute)
            {
                //See if we want this type of mission
                if ((Metatron.Config.MissionConfig.RunCourierMissions && cachedMission.Type.Contains(Type_Courier)) ||	//Check if we can run courier missions
                    CheckEncounterMission(cachedMission) ||																//Check if we can run encounter missions
                    (Metatron.Config.MissionConfig.RunTradeMissions && cachedMission.Type.Contains(Type_Trade)) ||		//Check if we can run trade missions
                    CheckMiningMission(cachedMission) ||																//Check if we can run mining missions
                    (Metatron.Config.MissionConfig.DoChainCouriers && cachedMission.Type.Equals(Type_ChainCourier)))	//Check if we can run storyline chain courier missions
                {
                    Metatron.Logging.LogMessage(ModuleName, methodName, LogSeverityTypes.Debug, "Mission \"{0}\" is acceptible.",
                        cachedMission.Name);
                    return true;
                }
            }

            var missionAgent = Metatron.AgentCache.GetCachedAgent(cachedMission.AgentId);
            Metatron.Logging.LogMessage(ModuleName, methodName, LogSeverityTypes.Debug, "Mission \"{0}\" for agent \"{1}\" ({2}) was not acceptible.",
        ...
        {
            return (Metatron.Config.MissionConfig.RunMiningMissions && cachedMission.Type.Contains("Mining") &&
                ((Metatron.Config.MissionConfig.DoOreMiningMissions && cachedMission.MiningMissionType == MiningMissionTypes.Ore) ||
                (Metatron.Config.MissionConfig.DoIceMiningMissions && cachedMission.MiningMissionType == MiningMissionTypes.Ice) ||
                (Metatron.Config.MissionConfig.DoGasMiningMissions && cachedMission.MiningMissionType == MiningMissionTypes.Gas)));
        }

        private bool CheckEncounterMission(CachedMission cachedMission)
        {
            if (Metatron.Config.MissionConfig.RunEncounterMissions && cachedMission.Type.Contains(Type_Encounter))
            {
                switch (cachedMission.FactionId)
                {
                    case (int)FactionIDs.Amarr_Empire:
                    ...
                    case (int)FactionIDs.Servant_Sisters_of_Eve:
                        return Metatron.Config.MissionConfig.DoEmpireKillMissions;
                    case (int)FactionIDs.Angel_Cartel:
                    case (int)FactionIDs.Blood_Raider_Covenant:
                    case (int)FactionIDs.Guristas_Pirates:
                    case (int)FactionIDs.Mordus_Legion_Command:
                    ...
                    case (int)FactionIDs.Unknown:
                        return Metatron.Config.MissionConfig.DoPirateKillMissions;
                    default:
                        //Probalby drones
        ...
        public bool IsAtMissionStartBookmark(CachedMission cachedMission)
        {
            ...
            var missionStartBookMark = Metatron.MissionCache.GetMissionStartBookmark(cachedMission);
            ...
            if (Metatron.Bookmarks.IsStationBookMark(missionStartBookMark))
            {
                if (Metatron.MeCache.InSpace || Metatron.MeCache.StationId != missionStartBookMark.ItemID)
                {
                    return false;
                }
            }
            else
            {
                ...
                if (DistanceTo(missionStartBookMark.X, missionStartBookMark.Y, missionStartBookMark.Z) >= (int)Ranges.Warp)
                {
                    ...
                    return Metatron.EntityProvider.EntityWrappers.Any(x => x.GroupID == (int)GroupIDs.WarpGate || x.TypeID == (int)TypeIDs.Beacon);
                }
            }
        ...
        public bool IsAtMissionEndBookmark(CachedMission cachedMission)
        {
            ...
            var missionEndBookmark = Metatron.MissionCache.GetMissionEndBookmark(cachedMission);
            ...
            //If the bookmark is a station and I'm not docked in it, I'm not there
            if (Metatron.Bookmarks.IsStationBookMark(missionEndBookmark))
            {
                if (Metatron.MeCache.InSpace || Metatron.MeCache.StationId != missionEndBookmark.ItemID)
                {
                    return false;
                }
            }
